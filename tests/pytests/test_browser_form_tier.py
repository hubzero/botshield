...
see:
  ...
  - Auto-submit form is broken (wrong method, wrong action, JS
    handler renamed).
  - Cookie attributes prevent the browser from echoing _bs_verified
...
def _has_verified_cookie(ctx) -> bool:
    return any(c["name"] == "_bs_verified" for c in ctx.cookies())
...
def test_silent_tier_js_pow_round_trip(bs_browser_context):
    ...
    # 2. The interstitial JS sets _bs_verified then
    ...
    assert _has_verified_cookie(ctx), (
        f"_bs_verified never landed in the browser cookie jar; "
