...
solves PoW, POSTs result, __Host-bs_session cookie is set, next page-load
...
def test_embedded_wrapper_mints_verified_cookie(
    ...
    with config_override(
        ...
        while time.monotonic() < deadline:
            ...
            if "__Host-bs_session" in cookies:
                cookie_present = True
                break
            time.sleep(0.25)

        assert cookie_present, (
            f"wrapper failed to mint __Host-bs_session within deadline; "
...
def test_embedded_turnstile_mints_verified_cookie(
    ...
    with config_override(
        ...
        assert cookie_present, (
            f"turnstile wrapper failed to mint __Host-bs_session within "
            f"deadline; cookies="
...
def test_embedded_hcaptcha_mints_verified_cookie(
    ...
    with config_override(
        ...
        assert cookie_present, (
            f"hcaptcha wrapper failed to mint __Host-bs_session within "
            f"deadline; cookies="
            f"{[c['name'] for c in bs_browser_context.cookies()]}"
        )
