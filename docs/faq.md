...
- **Allow-list.** When `BotShieldAllowVerifiedBots on` is set and the
  ...
  refresh ranges out of band with `services/bot-refresh/refresh-bot-ranges.sh`.
...
engine crawlers; `services/bot-refresh/refresh-bot-ranges.sh` fetches and updates
the CIDR files.
...
2. **Bot-range refresh script.** `services/bot-refresh/refresh-bot-ranges.sh`
