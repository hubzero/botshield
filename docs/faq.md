...
- **Allow-list.** When `BotShieldAllowVerifiedBots on` is set and the
  ...
  refresh ranges out of band with `services/refresh/refresh-bot-ranges.sh`.
...
engine crawlers; `services/refresh/refresh-bot-ranges.sh` fetches and updates
the CIDR files.
...
2. **Bot-range refresh script.** `services/refresh/refresh-bot-ranges.sh`
